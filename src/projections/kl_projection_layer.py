...
from projections.base_projection_layer import (
    BaseProjectionLayer,
    mean_projection,
    mean_equality_projection,
)
from projections.utils import gaussian_kl, get_numpy
...
class KLProjectionLayer(BaseProjectionLayer):
    def _trust_region_projection(
        ...
        **kwargs
        ...
        if self.mean_eq:
            proj_mean = mean_equality_projection(mean, old_mean, mean_part, eps)
        else:
        ...
        if not policy.contextual_std:
            ...
            proj_std = proj_std.expand(*mean.shape[:-1], -1, -1)
    ...
    def get_projection_op(batch_shape, dim: int, max_eval: int = MAX_EVAL):
        if not KLProjectionGradFunctionJoint.projection_op:
            KLProjectionGradFunctionJoint.projection_op = cpp_projection.BatchedProjection(
                batch_shape, dim, eec=False, constrain_entropy=False, max_eval=max_eval
            )
